...
public enum StockQuoteServiceFactory {
  ...
  public StockQuotesService getService(String provider,  RestTemplate restTemplate) {
    String service1 = "TIINGO";
    String service2 = "ALPHAVANTAGE";
    if (provider.equalsIgnoreCase(service1)) {
      StockQuotesService sqs = new TiingoService(restTemplate);
      return sqs;
    } else {
      StockQuotesService sqs = new AlphavantageService(restTemplate);
      return sqs;
    }
  }
}
